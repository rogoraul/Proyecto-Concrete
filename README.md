...
│   ├── 03_Modelos_Simple_2.ipynb
│   ├── 03_Modelos_Simple_3.ipynb
...
| Modelo Simple Red Neuronal | CNN propia | 233 | Train: 82.4%, Val: 82.7%, Test: 82.8% | Train: 2.5%, Val: 2.6%, Test: 2.4% | Train: 0.517, Val: 0.532, Test: 0.533 |
...
- **Regresión Logística:** Nos entrega probabilidades sobre la pertenencia a una clase. Para este modelo utilizamos la función de coste de **entropía cruzada** (Log Loss) porque penaliza exponencialmente las predicciones que son muy seguras y erróneas, optimizando la capacidad de clasificación probabilística. Hemos aplicado 3 modelos, uno sin preprocesamiento con un accuracy en Test del 56.4% y 4097 parámetros, uno aplicando HOG con un accuracy en Test del 53.4% y 1765 parámetros, y uno con HOG y PCA con un accuracy en Test del 52.2% y 518 parámetros.
- **Support Vector Machine (LinearSVC):** Este modelo encuentra el hiperplano óptimo que separa ambas clases. Utiliza como función de coste el **Hinge Loss** dado que su principal objetivo es maximizar el margen que separa las imágenes que tienen grieta de las que no. Hemos aplicado 3 modelos, uno sin preprocesamiento con un accuracy en Test del 53.7% y 4097 parámetros, uno aplicando HOG con un accuracy en Test del 53.0% y 1765 parámetros, y uno con HOG y PCA con un accuracy en Test del 51.0% y 518 parámetros.
- **Red Neuronal Convolucional:** Se ha diseñado una arquitectura de Red Neuronal Convolucional (CNN) muy simple, compuesta por una capa convolucional de 2 filtros con un kernel de tamaño (3x3) para extraer características básicas de la imagen como bordes y texturas, luego se ha añadido una capa de Global Average Pooling 2D para reducir la dimensionalidad, finalizando con una capa de salida de una neurona con activación sigmoide para la clasificación binaria (Grieta/sano). Durante el entrenamiento, se ha utilizado la funcionalidad de ModelCheckpoint para poder monitorizar la pérdida de validación y guardar los pesos de la mejor época con el máximo valor de accuracy en validación. 
- **CNN simple 2:** Para aumentar la complejidad respecto al modelo inicial que solo tenía una capa convolucional, se ha diseñado una arquitectura más profunda. Se han añadido tres capas convolucionales (de 64, 126 y 256 filtros), intercalados con capas de maxpooling. Para la clasificación se ha añadido una capa densa de 256 neuronas junto con una regularización de Dropout de 0.3 para controlar el sobreajuste. 
- **CNN simple 3:** Se mantiene la estructura de 3 capas convolucionales pero de (34, 64 y 128 filtros), lo que reduce drasticamente los parametros. Después de cada convolucion, se añade capas de BatchNormalization para estabilizar el entrenamiento. Para la clasificación se ha añadido una capa densa de 64 neuronas junto con una regularización de Dropout de 0.5 para controlar el sobreajuste. 
...
Los modelos clásicos (Lineal y SVM) no logran superar el 53% de rendimiento en las métricas clave. Esto confirma que, al aumentar la dificultad del dataset y la complejidad visual de las imágenes de hormigón, la extracción de características manual (como HOG o PCA) se vuelve insuficiente, haciendo estrictamente necesario el salto a arquitecturas de Deep Learning (CNNs). Ya que con el dataset de Özgenel, obteniamos aproximadamente un 98% de accuracy en HOG y PCA de ambos modelos. Por lo que al aumentar la complejidad, se puede observar que con modelos tan simples no se puede solucionar el problema. 

El Modelo Simple inicial sufre un colapso por el desbalanceo de clases, ya que al ser tan básico, predice de forma sistematica la clase mayoritaria, lo que muestra un accuracy engañoso del 82% pero un recall muy bajo del 2.4%, demostrando su nula utilidad real.

Al observar los modelos de CNN simple 2 y 3, vemos que logran un rendimiento más o menos decente y actúan como puente entre la red básica y las arquitecturas complejas de Transfer Learning. Ambos superan el 73% de Accuracy en validación. El Modelo Simple 2, que es el más grande, extrae mejores métricas generales (AUC de 0.778 en Test), mientras que el Modelo Simple 3 muestra que, gracias al BatchNormalization, puede acercarse a esos resultados siendo mucho más ligero a nivel de parametros. No obstante, ninguno de los dos logra alcanzar la capacidad de generalización que ofrece la extracción de características preentrenada de la ResNet50 (Modelos Complejos 2 y 3). 
